...
func TestRunReturnAll(t *testing.T) {
	...
	t.Run("successful command execution", func(t *testing.T) {
		out, _, err := exec.RunReturnAll("echo 'hello world'", 2)
		assert.Equal(t, 0, errors.GetCode(err))
	...
	t.Run("invalid command (bash-level error)", func(t *testing.T) {
		_, _, err := exec.RunReturnAll("invalid_command_xyz", 0)
		assert.Equal(t, 127, errors.GetCode(err)) // Bash 返回 127 表示命令未找到
		assert.NotNil(t, err)
	...
	t.Run("explicit non-zero exit code", func(t *testing.T) {
		_, _, err := exec.RunReturnAll("sh -c 'exit 5'", 2)
		assert.Equal(t, 5, errors.GetCode(err)) // 确定返回码
		assert.NotNil(t, err)
		assert.Contains(t, err.Error(), "exit-code not zero")
	...
	t.Run("command_start_failure", func(t *testing.T) {
		_, _, err := exec.RunReturnAll("", 2)
		assert.Equal(t, 125, errors.GetCode(err), "expected exit code 125 for empty command")
	...
	t.Run("signal termination", func(t *testing.T) {
		_, _, err := exec.RunReturnAll("sh -c 'kill -9 $$'", 2) // 使用 $$ 获取当前 shell 的 PID
		assert.Equal(t, 137, errors.GetCode(err))                                    // SIGKILL 对应退出码 128 + 9 = 137
		assert.NotNil(t, err)
		assert.Contains(t, err.Error(), "exit-code not zero")
	...
	t.Run("concurrent execution", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			go func() {
				defer wg.Done()
				_, _, err := exec.RunReturnAll("echo 'concurrent test'", 2)
				assert.Equal(t, 0, errors.GetCode(err))
				assert.Nil(t, err)
			}()
		}
	...
	t.Run("special characters in command", func(t *testing.T) {
		out, _, err := exec.RunReturnAll("echo 'special chars: !@#$%^&*()'", 2)
		assert.Equal(t, 0, errors.GetCode(err))
	...
	t.Run("timeout with no output", func(t *testing.T) {
		start := time.Now()
		out, _, err := exec.RunReturnAll("sleep 5", 1)
		duration := time.Since(start)
	...
	t.Run("command with stderr output", func(t *testing.T) {
		_, errOut, err := exec.RunReturnAll("sh -c 'echo error >&2'", 2)
		assert.Equal(t, 0, errors.GetCode(err))
	...
	t.Run("large output", func(t *testing.T) {
		...
		out, _, err := exec.RunReturnAll("dd if=/dev/zero bs=1M count=8 | base64", 30)
		assert.Equal(t, 0, errors.GetCode(err))
